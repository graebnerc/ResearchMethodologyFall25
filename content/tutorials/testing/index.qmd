...
The correct interpretation of confidence intervals is surprisingly subtle. Here are the right and wrong ways to talk about them:

**❌ INCORRECT formulations:**

- "There's a 95% probability that $\mu$ is between 240 EUR and 260 EUR"
- "I'm 95% confident that $\mu$ is between 240 EUR and 260 EUR"
- "We can be 95% certain that $\mu$ is between 240 EUR and 260 EUR"

**Why these are wrong:** Once you've calculated a specific interval from your data, the true parameter $\mu$ either is or isn't in that interval (probability = 1 or 0). The parameter is fixed, not random; the *interval* is what's random (it varies from sample to sample).

**✅ CORRECT formulations:**

- "The interval 240 EUR to 260 EUR was constructed using a procedure that captures the true parameter in 95% of samples"
...
**Acceptable shorthand (with caveats):**

- "We are 95% confident the true mean is between 240 EUR and 260 EUR"

This phrasing is widely used and generally accepted in practice, but it's somewhat imprecise. The word "confident" here refers to confidence in the *procedure*, not subjective certainty about this particular interval. If you use this shorthand, be aware that it's a convention that technically conflates procedure-level properties with interval-level statements.
...
However, in practice we almost never know $\sigma$v-vwe estimate it with our sample standard deviation $s$. This estimation adds extra uncertainty. The **t-distribution** accounts for this additional uncertainty:
...
- Similarly, "fail to reject $H_0$" ≠ "$H_0$ is true"
...
- **t-statistic:** $-1.77$ (sample mean is $1.77 SE$ below claimed value)
- **p-value:** $0.083$ (if $\mu = 85$, we'd see this difference 8.3% of the time)
- **Decision:** At $\alpha = 0.05$, fail to reject $H_0$ (insufficient evidence)
- **95% CI:** [78.6, 85.4] (notice that 85 is inside the interval)

**Business interpretation:** "While the sample mean (82) is lower than the claimed value (85), the difference isn't statistically significant at the 5% level. We don't have strong evidence that the true productivity differs from 85 units/day. However, the result is borderline ($p = 0.083$), suggesting we might want to collect more data or investigate further."
...
treatment <- rnorm(100, mean = 52, sd = 15)
...
- **t-statistic:** 3.30 (treatment mean is 3.30 SE higher than control)
- **p-value:** 0.0006 (very strong evidence)
- **Decision:** Reject $H_0$ (treatment effect is real)
- **95% CI:** [3.2, Inf] (one-sided, so only lower bound)

**Business interpretation:** "Customers who received the email campaign spent significantly more (mean = €52) than those who didn't (mean = 45 EUR). The difference of 7 EUR is statistically significant ($p < 0.001$), suggesting the campaign was effective. The 95% confidence interval suggests the true effect is at least 3.2 EUR."
...
intercept   86.4      4.32       20.0       0.000
price       -9.84     1.38       -7.15      0.000
```
...
A company tests two website designs with 100,000 users each:

- Design A: 2.01% conversion rate (2,010 conversions)
- Design B: 2.03% conversion rate (2,030 conversions)  
- Difference: 0.02 percentage points (20 additional conversions)

```{r}
#| echo: true

# Two-proportion test
prop.test(x = c(2010, 2030), n = c(100000, 100000))
```
...
- Does a 0.02% improvement justify redesign costs?
- Implementation time?
- Risk of unforeseen issues?
- Employee retraining?

If redesign costs are 50,000 EUR and each conversion generates 10 EUR profit:

- Additional profit: 20 conversions × 10 EUR = 200 EUR per 100,000 users
- This is tiny compared to implementation costs!
...
A company pilots a productivity training program with 10 employees:

- Before training: 75 units/day (average)
- After training: 90 units/day (average)
- Improvement: 15 units/day (20% increase!)
...
set.seed(789)
before <- rnorm(10, mean = 75, sd = 20)
after <- before + rnorm(10, mean = 15, sd = 10)  # each person improves

# Paired t-test
t.test(after, before, paired = TRUE)
```

**Result:** $p = 0.09$ (not statistically significant at $\alpha = 0.05$)

**But the effect is huge!** 20% productivity improvement would be extremely valuable company-wide.

**What went wrong?** Small sample ($n = 10$) + high variability = low power to detect effects

**Business decision:** Don't abandon the program! Instead:

- Expand the pilot to get more data
- Calculate ROI: if training costs 500 EUR/person and generates 5,000 EUR in productivity gains, it's worth it regardless of $p$-value
- Focus on effect size and confidence interval, not just significance

...
In this example, the coefficient for weeks (`r round(log_growth_coef, 3)`, see @tbl-logmodel-results) tells us users are growing at approximately `r weekly_growth_pct`% per week—going from around `r initial_users` users at launch to over `r format(final_users, big.mark = ",")` users after 20 weeks. This exponential pattern is typical in the early viral growth phase of successful apps like Instagram, TikTok, or WhatsApp before they hit market saturation.
